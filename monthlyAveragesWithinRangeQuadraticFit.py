...
def datetime_to_julian_day(dt):
    ...
    julian_day_number = (dt - epoch).days + 2440587.5
...
def plot_average_over_raw(station_index, start_year, end_year):
    ...
    # dates2 = datetime_to_julian_day(dates)
    # dates = dates2 - dates[0]
    ...
    # Add quadratic trend overlay
    if len(raw_dates) > 0:
        ordinals = [d.toordinal() for d in raw_dates]
        
        #z is the coefficients, in high to low
        z, cov = np.polyfit(ordinals, raw_levels, 2, cov=True)
        p = np.poly1d(z)
        plt.plot(raw_dates, p(ordinals), label='Quadratic Trend', color='yellow')
        
        # Calculate the uncertainties (standard deviations) of the coefficients
        uncertainties = np.sqrt(np.diag(cov))
        
        # Print the coefficients and their uncertainties
        print("Coefficients:")
        print("a:", z[0], "±", uncertainties[0])
        print("b:", z[1], "±", uncertainties[1])
        print("c:", z[2], "±", uncertainties[2])
    
    # confirm that z is the coefficients and which order is it in? high to low or low to high?
    
    
    plt.title(f'Raw Data and Monthly Average Water Level for {station_name} (Years: {start_year}-{end_year})',fontsize=16)
    plt.xlabel('Date')
...
"**********************************************************"
station_to_plot = 16  # Change this to the index of the station you want to plot
start_year = 1975
end_year = 2022
"***********************************************************"
